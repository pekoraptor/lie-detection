...
W pracy wykorzystano dwa zbiory danych, aby sprawdzić zdolność wybranego rozwiązania do nauki na danych przygotowanych w kontrolowanych warunkach laboratoryjnych, jak i jego generalizację w warunkach rzeczywistych (nagrania zbliżone jakością do tych, które będą zamieszczane przez użytkownika końcowego).
...
Finalnie wektory cech (trzymane wcześniej w tablicach NumPy) konwertowane są na tensory PyTorch, które są natywną strukturą danych dla operacji na karcie graficznej GPU, gdzie wykonywany jest trening sieci neuronowych.
