...
Na początku omówione zostanie, w jaki sposób kłamstwo formułowane jest w ludzkim umyśle i po czym można rozpoznać jego zaistnienie w zachowaniu niewerbalnym. Następnie zaprezentowany zostanie przegląd istniejących już rozwiązań w dziedzinie automatycznej analizy wiarygodności wypowiedzi (_state of the art_), co stanowi punkt odniesienia dla podejścia zaproponowanego w tej pracy. W dalszej części wytłumaczone zostaną podstawowe metody wizji komputerowej wykorzystywane do analizy cech, takich jak mimika twarzy czy ruchy ciała. Ostatnia sekcja przedstawi działanie wybranych algorytmów uczenia maszynowego, które zostały zastosowane w pracy jako modele klasyfikacyjne.
...
Początkowe próby zastosowania algorytmów uczenia maszynowego do automatycznej detekcji kłamstwa opierały się na klasycznych metodach wizji komputerowej. Dane zostawały poddane ręcznej ekstrakcji cech geometrycznych i tekstur, a następnie były używane do trenowania klasyfikatorów -- maszyn wektorów nośnych (SVM) lub drzew decyzyjnych. Przykładem takiego podejścia jest praca @pfister_2011, w której wykorzystano deskryptor LBP-TOP (_Local Binary Patterns from Three Orthogonal Planes_) do analizy czasoprzestrzennych zmian tekstury twarzy, używając klasyfikatora SVM. Rozwiązania te, choć przełomowe na tamten okres (ok. 2010-2015), były bardzo podatne na błędy wynikające z nieoptymalnych warunków oświetlenia i przemieszczeń kamery.

Przełom nastąpił wraz z popularyzacją głębokiego uczenia (ang. _deep learning_). Kluczową pracą z tego okresu jest publikacja Pérez-Rosas et al. @perez_rosas_real_life_2015, w ramach której utworzono zbiór Real-life Trial Dataset, zawierający nagrania z prawdziwych rozpraw sądowych. W swoich eksperymentach autorzy wykorzystali drzewa decyzyjne oraz Random Forest do klasyfikacji próbek składających się z cech językowych (wyekstrahowanych z transkrypcji) oraz wizualnych, m.in. trajektorii ruchów głowy i dłoni oraz detekcji uśmiechów. Badania te wykazały, że automatyczne systemy oparte na ekstrakcji cech wizualnych są w stanie osiągnąć skuteczność nawet 75%, znacząco przewyższając ludzką percepcję (wcześniej wspomniane 54%).
...
Równocześnie istniejący nurt badawczy wskazuje jednak, że to właśnie kanał wizualny dostarcza najwięcej unikalnych i trudnych do sfałszowania sygnałów (w postaci "przecieków emocjonalnych"), na co wskazuje literatura poświęcona detekcji mikroekspresji @wu_micro_expression_2017. Uzasadnia to skupienie się w niniejszej pracy na analizie wideo z wykorzystaniem metod wizji komputerowej w postaci m.in. Optical Flow oraz sekwencyjności rekurencyjnych sieci neuronowych, których podstawy teoretyczne zostaną omówione w kolejnych podrozdziałach.

== Metody wizji komputerowej w analizie twarzy <computer-vision-in-face-analysis>
Typowy potok analizy twarzy jest procesem wieloetapowym, mającym na celu przekształcenie surowego sygnału wideo w zbiór ustrukturyzowanych cech, możliwych do interpretacji przez algorytmy uczenia maszynowego. Proces ten, choć może różnić się szczegółami w zależności od konkretnej implementacji, z reguły opiera się na przetwarzaniu sekwencyjnym @sariyanidi_2015.

Pierwszym etapem jest akwizycja i wstępne przetwarzanie, polegające na pobraniu klatki z nagrania i konwersji przestrzeni barw. Następnie następuje detekcja twarzy, której celem jest lokalizacja obszaru zainteresowania i odrzucenie tła, co redukuje szum informacyjny. Na tak wyodrębnionym obszarze przeprowadza się wyrównanie oraz ekstrakcję cech. Na tym etapie wyznaczane są kluczowe cechy, takie jak m.in. położenie punktów morfologicznych (landmarków) i wektory ruchu (przepływ optyczny).
...
- *Metody gęste* (_dense_), obliczające wektor prędkości dla każdego piksela obrazu. Takie podejście jest zdecydowanie bardziej wymagające obliczeniowo, ale pozwala wykryć ruch nawet tam, gdzie nie występują wyraźne punkty charakterystyczne, dzięki czemu ma potencjał do wykrycia wystąpienia mikroekspresji.
...
Automatyczna ocena prawdomówności na podstawie wideo jest złożonym problemem klasyfikacji, który może zostać rozwiązany stosując różnorodne paradygmaty uczenia maszynowego. Wybór odpowiednich algorytmów w niniejszej pracy podyktowany był chęcią porównania dwóch fundamentalnie odmiennych podejść do analizy danych behawioralnych: podejścia opartego na statystykach opisowych oraz podejścia uwzględniającego pełną dynamikę czasową wypowiedzi.

W pierwszej kolejności omówiony zostanie algorytm Random Forest, który został wybrany jako reprezentant klasycznych metod uczenia maszynowego ze względu na swoją wysoką interpretowalność oraz odporność na szum w danych. W kontekście niniejszej pracy pełni on rolę modelu bazowego (ang. _baseline_), pozwalającego na weryfikację, czy proste, zagregowane cechy behawioralne są wystarczające do skutecznej detekcji kłamstwa. Następnie przedstawione zostaną rekurencyjne sieci neuronowe ze szczególnym uwzględnieniem architektury GRU, która została bezpośrednio wykorzystana w ramach tej pracy. Wybór modeli sekwencyjnych uzasadniony jest koniecznością analizy zależności czasowych między kolejnymi klatkami wideo, co pozwala na trafne odzwierciedlenie dynamicznej natury wypowiedzi. Wykonanie takiej analizy w oparciu wyłącznie o zagregowane statystyki opisowe (jak w przypadku Random Forest) jest niemożliwe. Na koniec, jako alternatywa dla finalnie wybranych algorytmów, przedstawiona zostanie krótka charakterystyka trójwymiarowych sieci konwolucyjnych (3D-CNN), które rozważano jako jedno z możliwych podejść do problemu oceny prawdomówności, lecz ostatecznie odrzucono ze względu na ograniczenia opisane w #link(<3D-CNN>)[poniższej sekcji o 3D-CNN].

=== Klasyczne metody uczenia maszynowego - Random Forest <classical-machine-learning-algorithms>
Do zrozumienia działania algorytmu Random Forest (Las Losowy) kluczowe jest poznanie idei drzew decyzyjnych @hastie_elements_2009. Jest to hierarchiczna struktura danych, służąca do podejmowania decyzji poprzez zadawanie sekwencji pytań o wartość cech (np. "Czy wartość wskaźnika EAR jest większa od 0,2?"). Powszechnie wizualizowana jest jako odwrócone drzewo, z korzeniem na górze, a liśćmi na dole, gdzie węzłami są kolejne pytania tworzące podział, a liście to finalne predykcje. 
...
Bramki wprowadzają znaczne usprawnienie działania rekurencyjnych sieci neuronowych, ale tym samym LSTM ma niezwykle dużo trenowalnych parametrów, przez co proces jego uczenia jest utrudniony i wymaga większej liczby danych treningowych. Architektura GRU (_Gated Recurrent Unit_) została opracowana przez Kyunghyuna Cho w celu optymalizacji i uproszczenia LSTM @cho_gru_2014. Łączy ona stan ukryty i stan komórki w jedno, ale korzysta z bramek w celu ochrony przed zanikającymi gradientami. W przeciwieństwie do LSTM, wykorzystującego trzy bramki, w GRU są tylko dwie:
...
Jako alternatywa dla RNN rozważono użycie trójwymiarowych konwolucyjnych sieci neuronowych (3D-CNN) @tran_c3d_2015. Klasyczne sieci konwolucyjne CNN (używane np. w YOLO) operują bezpośrednio na obrazach. Ich trójwymiarowy odpowiednik rozszerza operację splotu (konwolucji) o trzeci wymiar -- czas. Takie podejście eliminuje potrzebę skomplikowanego przetwarzania wstępnego, ponieważ sieć bezpośrednio z nagrań uczy się jednocześnie wyglądu obiektów i ich ruchu. Nie jest to jednak rozwiązanie bez wad. Tak skomplikowany model jest bardzo ciężki obliczeniowo, a efektywne wytrenowanie go wymaga tysięcy, a nawet milionów nagrań. Dziedzina detekcji kłamstwa cierpi na deficyt danych, a publicznie dostępne zbiory liczą zaledwie setki próbek. Z tego powodu zdecydowano się na porzucenie prób modelowania opartych na tej architekturze.