...
Eksperyment sugeruje, że kłamstwo naturalne wiąże się z zupełnie innym zachowaniem niż kłamstwo wymuszone. Random Forest o wiele lepiej radzi sobie z klasyfikacją nagrań zawierających kłamstwa high-stakes, chociaż nierównowaga klas w zbiorze @silesian mógł mocno wpłynąć na działanie modelu wytrenowanego na jego nagraniach.
...
- parametry geometryczne ramki twarzy (`Box_Center_Y`, `Box_Width`) oraz cechy obrazujące ogólny ruch między klatkami, co w przypadku nagrań ze statycznej kamery i z jednolitym tłem wskazuje na zmianę postury ciała -- wiercenie się na krześle, pochylanie w przód-tył i ogólnie wysoką ruchliwość ciała,
- cechy związane z mimiką (`Mouth`, `Eye` i `Brow`), być może wskazujące na mikroekspresje.
...
Jak przedstawiono na #link(<overfit-check-learning-curve>)[powyższej krzywej uczenia], model bardzo szybko zredukował stratę do wartości bliskich zeru (w 100 epok). Wszystkie próbki zostały bezbłędnie sklasyfikowane, o czym świadczy perfekcyjna dokładność równa `1.0`. Dzięki temu potwierdzono, że dane poprawnie płyną przez wszystkie warstwy sieci, a gradienty są precyzyjnie obliczane i aktualizują wagi w przewidywany sposób. Oznacza to także, że architektura ma wystarczającą pojemność informacyjną, żeby nauczyć się zależności między cechami a etykietami. Pozytywny wynik testu pozwolił na rozpoczęcie właściwego procesu uczenia na pełnym zbiorze treningowym.
...
Celem tego podrozdziału jest głębsze zbadanie zdolności generalizacji modelu i weryfikacja hipotezy o negatywnym wpływie laboratoryjnych wymuszonych kłamstw low-stakes na skuteczność predykcji. Przeprowadzono transfer wiedzy modelu wytrenowanego na zbiorze @silesian, dotrenowując go na @real_life_ddd.
...
Jak można było się spodziewać, model nie osiągnął zadowalających wyników. Podobnie jak w przypadku ewaluacji na zbiorze testowym z @silesian, przewidywał on kłamstwo dla większości próbek. Wartość AUC jest porównywalna z tą uzyskaną na podzbiorze testowym zbioru oryginalnego, a niższą dokładność w tym przypadku można wytłumaczyć równomiernym rozkładem etykiet. Ewaluacja zero-shot wskazuje na to, że model wytrenowany na danych laboratoryjnych nie jest dobrym klasyfikatorem dla danych rzeczywistych bez uprzedniego dotrenowania.
...
Osiągnięte AUC przewyższyło to uzyskane na oryginalnym zbiorze danych (@silesian), ale jest niższe niż to uzyskane przez Random Forest. Dowodzi to, że wskaźniki kłamstwa są znacznie wyraźniejsze w sytuacjach _high-stakes_ (sala sądowa) niż w _low-stakes_ (laboratorium). Zamrożony ekstraktor cech (BiGRU) skutecznie wykrywa te sygnały, a nowa głowa klasyfikacyjna nauczyła się je interpretować. Warto jednak zwrócić uwagę na niską dokładność przy bardzo wysokiej czułości. Prawdopodobnie wynika to z faktu, że w warunkach sądowych stres, jak i obciążenie poznawcze towarzyszy zarówno kłamcom, jak i osobom prawdomównym. Model, wykrywając silne napięcie, klasyfikuje większość osób jako podejrzane, z czego wynika także niski próg decyzyjny (`0.13`). Mimo to, wysokie AUC dowodzi, że model poprawnie nadaje wyższe prawdopodobieństwo kłamstwa rzeczywistym kłamcom, co jest widoczne także na #link(<rl-finetuned-prob-histogram>)[poniższym histogramie], gdzie rozkłady prawdopodobieństw klas są od siebie lepiej odseparowane niż w przypadku wyników na oryginalnym zbiorze.
