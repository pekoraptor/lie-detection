...
Treść tego rozdziału rozpocznie się od definicji wymagań funkcjonalnych i niefunkcjonalnych. Następnie omówiony zostanie stos technologiczny wraz z argumentacją za jego wyborem oraz opis architektury. Na końcu zaprezentowane będzie działanie stworzonego prototypu. 
...
- *WN2. Wydajność:* Czas przetwarzania powinien być zminimalizowany poprzez zastosowanie downsamplingu.
...
W procesie projektowania systemu skupiono się na modularności i separacji odpowiedzialności, co ma na celu zachowanie czytelności kodu oraz łatwość jego rozszerzania w przyszłości o dodatkowe funkcjonalności. Logika aplikacji została podzielona na trzy warstwy, separując logikę interfejsu graficznego, przetwarzania danych i inferencji.
...
Za wstępne przetworzenie nagrań i konwersję do formy akceptowanej przez modele odpowiadają moduły pomocnicze. Klasa `VideoProcessor` dokładnie odtwarza logikę potoku przetwarzania obrazu wykonywanego przed trenowaniem modeli. Funkcje zdefiniowane w pliku `features.py` wyciągają inteligentne (ang. _feature-engineered_) wskaźniki, obliczając znormalizowane dystanse między punktami charakterystycznymi twarzy, konstruując cechy pochodne (prędkościowe), aplikując filtry wygładzające (średnia krocząca) i odpowiednio skalując dane.

Logika samego interfejsu graficznego zaimplementowana została w pliku `app.py`. Moduł ten pełni rolę orkiestratora systemu -- definiuje układ komponentów wizualnych, obsługuje interakcję z użytkownikiem, wywołuje odpowiednie funkcje pomocnicze i predykcje modeli, a także odpowiada za wizualizację wyników. Ważnym aspektem architektury jest optymalizacja wydajności. Wykorzystano mechanizm buforowania (ang. _caching_), aby zapobiec wielokrotnemu wczytywaniu wag modeli do pamięci operacyjnej. Ładowanie to jest wykonywane tylko raz (przy uruchomieniu strony) oraz po każdej zmianie wybranego modelu.
...
#figure(
  ...
  caption: [Widok główny aplikacji w fazie konfiguracji. W panelu bocznym widoczny wybór modelu, a w części centralnej -- podgląd analizowanego nagrania.]
) <ui-uploaded-video>
