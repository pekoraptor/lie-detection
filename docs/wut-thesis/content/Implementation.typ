...
- `opencv-python` -- do operacji na obrazach, w tym wczytywania wideo, skalowania, obliczania Optical Flow algorytmem Farnebacka,
...
W przeciwieństwie do sieci rekurencyjnych, Random Forest nie obsługuje danych sekwencyjnych o zmiennej długości. Ze względu na ten wymóg sekwencyjne wektory cech musiały zostać spłaszczone do wektorów o stałym rozmiarze, reprezentujących całe nagranie. W tym celu dla każdej z 23 cech wyznaczono cztery statystyki opisowe: średnią arytmetyczną, odchylenie standardowe, maksimum i zakres wartości. Operacja ta pozwoliła na transformację zmiennej liczby klatek w stały wektor o rozmiarze 92 ($23 "cechy" times 4 "statystyki"$), który był następnie podawany na wejście klasyfikatora.

=== Konfiguracja modelu <rf-model-configuration>
Klasa `RandomForestClassifier` jest bardzo konfigurowalna -- pozwala na znaczne dostosowanie algorytmu do własnych potrzeb. Hiperparametry zostały dobrane eksperymentalnie, a oto ich wartości:
...
Wektor kontekstu uzyskany z modułu atencji trafia do perceptronu wielowarstwowego, który składa się z następujących warstw:
- `Linear(32, 16)`, 
- `ReLU`,
- `Dropout(0.4)`,
- `Linear(16, 1)`.
Wąskie gardło (rozmiar 16) przed ostateczną predykcją wymusza na sieci ekstrakcję najważniejszych cech. Model zwraca surowe logity (kształt wyjścia: `[Batch, 1]`), gdyż wykorzystana podczas treningu funkcja straty `BCEWithLogitsLoss` ma wbudowaną funkcję sigmoidalną.
...
Zdecydowano się na użycie optymalizatora `AdamW`, który jest poprawioną implementacją klasycznego optymalizatora `Adam`. Wprowadza on bardziej efektywne zanikanie wag (ang. _weight decay_), rozdzielając je od aktualizacji gradientu. Dzięki temu regularyzacja modelu jest silniejsza bez pogarszania zbieżności treningu, co jest kluczowe przy małych zbiorach danych.
