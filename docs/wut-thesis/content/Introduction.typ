...
W przeciwieństwie do metod tradycyjnych, metody oparte na _wizji komputerowej_ (ang. _computer vision_) oferują podejście bezkontaktowe. Wykorzystanie kamer oraz algorytmów uczenia maszynowego pozwala na analizę nagrań wideo osób bez konieczności ingerencji fizycznej. Badaniu podlegają subtelne zmiany w mimice twarzy, ruchy oczu, gesty rąk czy postawa ciała, które mogą dostarczać wskazówek dotyczących prawdomówności. Takie podejście jest mniej stresujące dla badanych, bardziej dyskretne i może być stosowane w szerszym zakresie sytuacji, np. podczas rozmów kwalifikacyjnych, przesłuchań czy monitoringu bezpieczeństwa @perez_rosas_real_life_2015, @radlak_bozek_2015.
...
== Cel pracy <goal>
Głównym celem niniejszej pracy inżynierskiej było zaimplementowanie kompletnego systemu informatycznego wykorzystującego metody wizji komputerowej i algorytmy sztucznej inteligencji do wspierania człowieka w ocenie wiarygodności wypowiedzi na podstawie analizy nagrań wideo. Cel ten został zrealizowany w aspekcie badawczym oraz aplikacyjnym.

Celem badawczym było zbadanie, czy cechy wizualne (mimika, ruchy głowy i reszty ciała, itd.) są wystarczająco informatywne, aby umożliwić skuteczną klasyfikację prawdomówności wypowiedzi z dokładnością przewyższającą ludzką percepcję. W tym celu przeprowadzono eksperymenty porównujące różne podejścia modelowe oraz metody ekstrakcji cech z filmów. Dodatkowo zbadano wpływ technik takich jak transfer learning czy inżynieria cech na zdolność generalizacji modeli klasyfikacyjnych.
...
+ stworzenie interfejsu dla użytkownika końcowego z wykorzystaniem frameworku Streamlit, umożliwiającego praktyczne zastosowanie ostatecznie wybranego rozwiązania.
...
- *Rozdział piąty* składa się z opisu przeprowadzonych eksperymentów badawczych, analizy uzyskanych wyników, w tym: metryk skuteczności, analizy istotności cech, wpływu inżynierii cech i transfer learningu na efektywność modeli.
...
- W *rozdziale siódmym* zawarte są wnioski końcowe z realizacji projektu, ocena osiągniętych rezultatów oraz propozycje dalszego rozwoju i badań w tym obszarze.