...
class VideoProcessor:
    def __init__(self):
        ...
        try:
            ...
            self.emotion_detector = EmotionRecognition(device=emo_device)

        finally:
            torch.load = original_load

        if hasattr(self.face_detector, "to"):
    ...
    def _geometric_normalization(self, frame, landmarks_objects):
        ...
        left_eye = np.array(
            [
                [landmarks_objects[i].x * w, landmarks_objects[i].y * h]
    ...
    def _get_emotion_probs(self, frame):
        ...
        tensor = (
            self.emotion_detector.transform(frame)
            .unsqueeze(0)
            .to(self.emotion_detector.device)
        )
        ...
        return {
            self.emotion_detector.emotions[i]: float(probs[i])
        ...
        while current_frame <= end_frame:
            ...
            if landmarks is None:
                ...
                landmarks_flat = np.array(
                    [(p.x, p.y) for p in landmarks], dtype=np.float32
                ).flatten()
